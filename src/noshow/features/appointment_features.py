...
def add_days_since_created(appointments_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    appointments_df["days_since_created"] = pd.to_datetime(appointments_df["created"], format="%Y-%m-%d %H:%M:%S", errors="coerce")
...
def add_minutes_early(appointments_df: pd.DataFrame, cutoff: int = 60) -> pd.DataFrame:
    ...
    appointments_df : pd.DataFrame
        ...
        level called `start` and a datetime column `arrival`
    cutoff : int, optional
    ...
    appointments_df["minutes_early"] = (
        ...
        - appointments_df["arrival"]
    ).dt.total_seconds() / 60

    appointments_df.loc[appointments_df["arrival"].isna(), "minutes_early"] = 0
